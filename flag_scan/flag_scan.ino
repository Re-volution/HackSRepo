/*
 * 项目：ESP32-C3 摔倒报警器（三段状态机版 + AP配网 + 心跳 + 中断唤醒）
 * 功能：检测自由落体+撞击+长时间静止，避免误触发
 *       支持按键开启AP模式，通过网页配置WiFi
 *       每20小时发送一次心跳到服务器
 *       静止时深度睡眠，运动时中断唤醒（超低功耗）
 * 硬件：ESP32-C3 + MPU6050 + 蜂鸣器 + LED
 * I2C：SDA→GPIO4, SCL→GPIO5
 * 中断：MPU6050 INT → GPIO2
 * 蜂鸣器：GPIO10
 * LED：GPIO8
 * 按键：GPIO9
 */
...
const int wakeupPin = 2;           // MPU6050 INT 引脚（GPIO2，RTC GPIO）
...
const int INT_ENABLE = 0x38;       // 中断使能寄存器
const int MOT_THRESHOLD = 0x1F;    // 运动检测阈值寄存器
const int MOT_DURATION = 0x20;     // 运动检测持续时间寄存器
...
// ========== 进入深度睡眠 ==========
void goToDeepSleep() {
  Serial.println("进入深度睡眠，等待运动中断唤醒...");
  digitalWrite(ledPin, LOW);
  digitalWrite(buzzerPin, HIGH);
  
  // 配置 MPU6050 运动中断
  enableMPU6050MotionInterrupt();
  ...
  // 设置 GPIO2 为唤醒源（高电平唤醒）
  // GPIO2 是 RTC GPIO，支持深度睡眠唤醒
  esp_deep_sleep_enable_gpio_wakeup(BIT64(GPIO_NUM_2), ESP_GPIO_WAKEUP_GPIO_HIGH);
...
// ========== 配置 MPU6050 运动检测中断 ==========
void enableMPU6050MotionInterrupt() {
  // 唤醒 MPU6050
  ...
  // 设置运动检测阈值（约 10mg，非常灵敏）
  Wire.beginTransmission(MPU_ADDR);
  Wire.write(MOT_THRESHOLD);
  Wire.write(10);
  Wire.endTransmission();
  
  // 设置运动检测持续时间
  Wire.beginTransmission(MPU_ADDR);
  Wire.write(MOT_DURATION);
  Wire.write(3);
  Wire.endTransmission();
  ...
  Serial.println("MPU6050 运动检测中断已使能");
}
...
// ========== 进入低功耗循环 ==========
void enterLowPowerLoop() {
  Serial.println("进入低功耗模式，等待运动中断...");
...
void setup() {
  ...
  Serial.println("=== 摔倒报警器启动（三段状态机 + AP配网 + 心跳 + 中断唤醒）===");
  ...
  Serial.println("提示：静止时自动进入低功耗模式，运动时唤醒");
}
...
void loop() {
      ...
      if (!significantMove) {
        ...
        if (stillDuration >= STILL_REQUIRED_MS) {
          ...
          if (!hasSent && (millis() - lastSentTime > COOLDOWN_SECONDS * 1000)) {
            ...
            lastSentTime = millis();
            hasSent = true;
            Serial.println("报警已发送，5秒后进入低功耗模式...");
            delay(5000);
            // 报警后进入低功耗模式
            enterLowPowerLoop();
          }
  ...
  // ========== 空闲时进入低功耗模式 ==========
  // 如果当前是 IDLE 状态且没有待处理的事件，进入低功耗模式
  if (fallState == IDLE && !apModeActive) {
    enterLowPowerLoop();
  }
